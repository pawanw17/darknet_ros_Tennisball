...
void YoloObjectDetector::init()
{
  ...
  std::string BallerTopicName;
  int BallerQueueSize;
  bool BallerImageLatch;
  ...
  nodeHandle_.param("publishers/baller/topic", BallerTopicName,
                    std::string("baller"));
  nodeHandle_.param("publishers/baller/queue_size", BallerQueueSize, 1);
  nodeHandle_.param("publishers/baller/latch", BallerImageLatch, false);
...
baller_ = nodeHandle_.advertise<darknet_ros_msgs::TennisNeuron>(BallerTopicName,
                                                BallerQueueSize,
                                                BallerImageLatch);
  // Action servers.
...
void *YoloObjectDetector::publishInThread()
{
  ...
  if (num > 0 && num <= 100) {
    ...
    msg.header.frame_id = "no. of People";
    //msg.count = num;
    msg.count = rosBoxCounter_[0];
	...
	// edit here probably
	  if(i==49 ||i==32 || i==47 || i==46)
		{
		tennisBallResults_.probability = rosBoxes_[i][j].prob;
		tennisBallResults_.breadth = xmax - xmin;
		tennisBallResults_.length = ymax - ymin;
		tennisBallResults_.ball = 42;	
		tennisBallResults_.Class = tennismsg.data;
 		}
	  else
		{
	        tennisBallResults_.probability = 0;
                tennisBallResults_.breadth = 0;
 	        tennisBallResults_.length = 0;
   		tennisBallResults_.ball = 0;
		tennisBallResults_.Class = " ";
		}
	  //numnum.data = 69;
	  //else 
	  //numnum.data = 0;
    ...
    //numnum.data = ball.data;
    //baller_.publish(numnum);
    //numnum.data=0;
    baller_.publish(tennisBallResults_);
} else {
    darknet_ros_msgs::ObjectCount msg;
    msg.header.stamp = ros::Time::now();
